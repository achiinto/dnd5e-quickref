data_movement = [
    {
        title: "移動 Move",
        icon: "run",
        subtitle: "消耗: 每 5 呎 消耗 5 呎",
        description: "移動消耗: 每 5 呎 消耗 5 呎",
        reference: "PHB, pg. 190.",
        bullets: [
            "如果你已經移動了多過一次, 例如你的步行速度和飛行速度, 你可以在你的移動期間在不同的移動型式間變換。當你需要轉換移動方式時，在剩餘的可移動力中減去你已經動用了的移動消費便可。",
            "你可以移動經過對你無惡意的生物所處之空間。",
            "你只可以移動經過對你有惡意的生物所處之空間，如果對方至少比你大又或者細你兩個身位。",
            "另一個生物的所處空間對你來說是一個「困難地形」（difficult terrain)。",
            "無論那生物是友好又或是敵對，你都不能夠在其身處空間完結你的移動回合。"
        ]
    },
    {
        title: "攀爬 Climb",
        icon: "crags",
        subtitle: "消耗: 每 5 呎 消耗 10 呎",
        description: "移動消耗: 每 5 呎 消耗 10 呎攀爬",
        reference: "PHB, pg. 182.",
        bullets: [
            "如果攀爬的動作要求是艱難時，可以使用力量「運動」測試 （Strength Athletics)"
        ]
    },
    {
        title: "游泳 Swim",
        icon: "at-sea",
        subtitle: "消耗: 每 5 呎 消耗 10 呎",
        description: "移動消耗: 每 5 呎 消耗 10 呎游泳",
        reference: "PHB, pg. 182.",
        bullets: [
            "如果游泳的動作要求是艱難時，可以使用力量「運動」測試 （Strength Athletics)"
        ]
    },
    {
        title: "倒下伏地 Drop prone",
        icon: "falling",
        subtitle: "消耗 0 呎",
        description: "移動消耗: 0 呎 (免費)",
        reference: "PHB, pgs. 190-191,292.",
        bullets: [
            "你可以將身體倒下在地上而無需使用任何你的移動力",
            "在倒地後的情況下，你必需要以爬行或魔法幫助下（如瞬間轉移）來移動",
            "倒下在地上會對角色加上「伏地」的狀況。（對你使用的近身攻擊能有「優勢」，對你使用的遠距離攻擊有「劣勢」，你自己作出的攻擊有「劣勢」）"
        ]
    },
    {
        title: "爬行 Crawl",
        icon: "crawl",
        subtitle: "消耗: 每 5 呎 消耗 10 呎",
        description: "移動消耗: 每 5 呎 消耗 10 呎爬行",
        reference: "PHB, pg. 182.",
        bullets: [

        ]
    },
    {
        title: "站起來 Stand up",
        icon: "strong",
        subtitle: "消耗: 角色一半的移動速度",
        description: "移動消耗: 角色一半的移動速度",
        reference: "PHB, pg. 190-191.",
        bullets: [
            "如果你並沒有足夠的移動力又或者你的移動速度是零，你就不能夠站起來"
        ]
    },
    {
        title: "跳高 High jump",
        icon: "wingfoot",
        subtitle: "消耗: 每 5 呎 消耗 5 呎",
        description: "移動消耗: 每 5 呎 消耗 5 呎 跳躍",
        
        reference: "PHB, pg. 182.",
        bullets: [
            "當你至少用腳行走了10呎遠後，你可以向上跳起至3 + 力量加減度 (Strength modifier)的呎高。",
            "如果你在沒有助跑的情況下原地跳高，你只能夠跳起至一半的高度。",
            "當你跳起時，你可以把雙手提升至你自己身高的一半。",
            "在某些情況下，你的地下城主可能會准許你做一次力量「運動」測試 （Strength Athletics)來決定你可否跳得比你平常所跳得高。"
        ]
    },
    {
        title: "跳遠 Long jump",
        icon: "wingfoot",
        subtitle: "消耗: 每 5 呎 消耗 5 呎",
        description: "移動消耗: 每 5 呎 消耗 5 呎 跳躍",
        reference: "PHB, pg. 182.",
        bullets: [
            "當你至少用腳行走了10呎遠後，你可以向前跳遠至你的體力值 (Strength score)的呎數距離。",
            "如果你在沒有助跑的情況下原地跳遠，你只能夠跳遠至一半的距離。",
            "你做以一次難度 DC 10 的力量度「運動」測試 （Strength Athletics)來跳過一個低矮的障礙物，一個不高於跳躍距離的四分之一高度。如果測試失敗，你撞上那個障礙物。",
            "你做以一次難度 DC 10 的敏捷度「雜技」測試 （Dexterity Acrobatics)來以腳足成功落地於不平面的「困難地形」上。如果測試失敗，你跌倒在地上成伏地狀態。"
        ]
    },
    {
        title: "除意創意 Improvise",
        icon: "juggler",
        subtitle: "不在此清單上的任何特技",
        description: "執行你可以想像的任何動作或特技",
        bullets: [
            "當你所形容的動作沒有在規則中存在，你的地下城主會跟你說明是否可能並且有需要投什麼骰來決定成敗。"
        ]
    },
    {
        title: "困難的地形 Difficult terrain",
        icon: "stone-pile",
        subtitle: "消耗修正: 每 5 呎 加 5 呎",
        reference: "PHB, pg. 182.",
        description: "在困難的地形上移動，需要每 5 呎 多消耗 5呎的移動力",
        bullets: [
        ]
    },
    {
        title: "抓捕移動 Grapple move",
        icon: "grab",
        subtitle: "消耗修正: 速度減半",
        description: "拖動或攜帶被抓的生物",
        reference: "PHB, pg. 195.",
        bullets: [
             "當你抓捕着另一個生物一同移動時，你的速度降低至平時的一半。除非那個生物的身體是你的兩個身碼或更少以下。",
             "請參考攻擊行動去更明白如何去抓捕一個生物。"
        ]
    }
]
